...
app.prepare().then(() => {
  ...
  const checkWinCondition = (gameId: string) => {
    ...
    const activeImpostors = game.players.filter(
      (p) => p.role === 'impostor' && p.status !== 'ghost',
    );
  ...
  const tallyVotes = (gameId: string, nominatedPlayerId: string) => {
    ...
    if (votesForDrop >= votesForRemain && nominatedPlayer) {
      ...
      io.to(gameId).emit('vote_ended', {
        eliminatedPlayer: nominatedPlayer,
        outcome: 'eliminated',
      });
    } else {
  ...
  io.on('connection', async (socket) => {
    ...
    if (game) {
      ...
      if (player) {
        ...
        const playerFromDb = await db.query.players.findFirst({
    ...
    socket.on('summon_gathering', ({ nominatedPlayerId }: { nominatedPlayerId: string }) => {
      ...
      if (
        ...
        game.status !== 'in-progress' ||
        !initiator ||
        initiator.isObserver ||
        initiator.isGatheringSummoned
      ) {
        return;
      }
    ...
    socket.on(
      'submit_vote',
      ({ nominatedPlayerId, choice }: { nominatedPlayerId: string; choice: 'drop' | 'remain' }) => {
        const game = getGame(gameId);
        if (!game || !game.votes) return;
        ...
        const activePlayers = game.players.filter(
          (p) => p.status === 'active' && !p.isObserver,
        ).length;
